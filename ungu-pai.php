<?php
/**
 * BLUE — Secure File Manager (single-file, legacy-friendly)
 ...
 * Security
 * --------
 * - Session-based auth with CSRF tokens for POST actions.
 * - Anti-indexing headers: noindex, nofollow, nosnippet, noimageindex.
 * - X-Frame-Options: DENY and X-Content-Type-Options: nosniff.
 * - Simple credential gate (username + password hash). Update the constants below.
 ...
 *     define('AUTH_PASS_HASH', '...'); // bcrypt via password_hash()
 * - Optional: adjust UI (Tailwind), fonts, or preview size limits.
 *
 * Notes
 * -----
...
/* ---------- Auth ---------- */
define('AUTH_USER', 'admin');
define('AUTH_PASS_HASH', '$2y$10$SPGA/8ABv8AY.d5TXNNSeunxVqnql9sEC9st3jXOOl0xHtDsZw95q'); // bcrypt of your password
...
function render_login($err=''){
    ...
    <!doctype html><html lang="en" class="dark"><head>
      ...
      <style>
        ...
        .btn{background:linear-gradient(180deg,#ea00ff,#ea00ff);color:#eaf2ff;border-radius:12px;padding:.7rem 1rem;font-weight:700;transition:transform .05s, box-shadow .15s, filter .15s; box-shadow:0 6px 20px rgba(37,99,235,.25);}
...
<head>
  ...
  <style>
    ...
    .field:focus{outline:none;box-shadow:0 0 0 4px rgba(99,102,241,.25);border-color:#ea00ff}
    .btn{background:linear-gradient(180deg,#ea00ff,#ea00ff);color:#eef2ff;border-radius:10px;padding:.5rem .75rem;font-weight:700;font-size:.875rem;line-height:1.25rem;display:inline-flex;align-items:center;justify-content:center;transition:transform .05s, box-shadow .15s, filter .15s; box-shadow:0 6px 20px rgba(99,102,241,.22);}
